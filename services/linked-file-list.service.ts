...
const getBrokenName = (fullFileName: string) => {
  return {
    fileName: fullFileName.slice(4, -4),
    fileCode: fullFileName.slice(0, 4),
    nextCode: fullFileName.slice(-4),
  };
};

const orderBrokenNames = (
  startCode: string,
  brokenNames: Array<BrokenName>
) => {
  const newNames: Array<BrokenName> = [];
...
const findNodeIndex = (code: string, brokenNames: Array<BrokenName>) => {
...
export class LinkedFileList {
  ...
  public static constructFromFolder(folder: string): LinkedFileList {
    ...
    let rootFileName: BrokenName | null = null;
    const brokenNames: Array<BrokenName> = [];
    for (let i = 0; i < files.length; i++) {
      if (files[i].startsWith(this.rootCode)) {
        rootFileName = getBrokenName(files[i]);
      } else {
        brokenNames.push(getBrokenName(files[i]));
      }
    }
    if (!rootFileName) {
